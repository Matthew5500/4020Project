...
window.AA = (function () {
  ...
  // Treat all server LocalDateTime values as UTC when converting to JS Date.
  // The mid-tier stores endTime as a timezone-less value using the server's
  // timezone (typically UTC). When the browser sees a string like
  // "2025-11-28T21:40:00" it would normally assume LOCAL time, which causes
  // the countdown to disagree with the server. By appending "Z" when there is
  // no timezone information, we interpret it as UTC and stay in sync.
  ...
  function parseServerDate(iso) {
    ...
    // If the string has no timezone info (e.g. "2025-11-28T21:40:00"),
    // assume the server meant UTC and append "Z" so Date parses it
    // consistently.
    if (!SERVER_TZ_REGEX.test(str)) {
      candidate = str + "Z";
    }
